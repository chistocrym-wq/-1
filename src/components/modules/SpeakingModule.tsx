  ...
  if (finished) {
    ...
    return (
      <div className="animate-scale-in flex flex-col items-center justify-center py-12">
        ...
        <div className="flex gap-3"><button onClick={handleRetry} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-slate-100 text-slate-700 font-medium hover:bg-slate-200"><RotateCcw className="w-4 h-4" /> Пройти заново</button><button onClick={onBack} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-slate-900 text-white font-medium hover:bg-slate-800">К модулям</button></div>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade-in">
      ...
      {task.type === 'introduction' ? (
        <>
          <div className="flex items-center justify-end mb-2"><button type="button" onClick={() => setShowSecrets(true)} className="text-xs sm:text-sm font-medium text-slate-500 hover:text-rose-700 transition-colors px-2 py-1">Секреты сдачи</button></div>
          <div className="border border-slate-300 bg-white mb-4 overflow-hidden">
            {task.prompts.map((prompt, index) => <div key={prompt} className={cn('min-h-[58px] sm:min-h-[64px] flex items-center justify-center px-4 text-center text-lg sm:text-xl font-medium text-slate-800', index !== 0 && 'border-t border-slate-300')}>{prompt}</div>)}
          </div>
          <div className="border border-slate-300 bg-white h-24 mb-4" aria-label="Otto" />
        </>
      ) : (
        <div className="rounded-2xl border border-slate-200 bg-white p-5 mb-4"><h4 className="font-semibold text-slate-900 mb-3">Wählen Sie ein Thema</h4><div className="space-y-2">{task.prompts.map((prompt, index) => <button key={prompt} onClick={() => { setSelectedPrompt(index); setShowSample(false); }} className={cn('w-full text-left p-3 rounded-xl border transition-colors text-sm', selectedPrompt === index ? 'bg-rose-50 border-rose-300 text-rose-800' : 'bg-slate-50 border-slate-100 text-slate-700 hover:border-rose-200')}><span className="font-semibold mr-2">{index + 1}.</span>{prompt}</button>)}</div></div>
      )}

      <SpeakingRecorder key={`${task.id}-${activePrompt ?? 'intro'}`} task={taskForAI} points={requiredPoints} onChecked={(score) => { setPracticed((previous) => { const next = [...previous]; next[currentTask] = true; return next; }); setScores((previous) => { const next = [...previous]; next[currentTask] = score; return next; }); }} />

      {task.sampleAnswer && <div className="rounded-2xl border border-slate-200 bg-white p-5 mb-6"><button onClick={() => setShowSample((value) => !value)} className="flex items-center gap-2 text-sm font-medium text-rose-700"><Eye className="w-4 h-4" /> {showSample ? 'Скрыть пример ответа' : 'Показать пример ответа'}</button>{showSample && <div className="mt-4 p-4 rounded-xl bg-rose-50 border border-rose-200"><p className="text-xs font-medium text-rose-700 mb-2">Beispiel / Пример:</p><p className="text-slate-700 leading-relaxed">{task.sampleAnswer}</p></div>}</div>}
      <div className="flex justify-end"><button onClick={handleNext} disabled={!practiced[currentTask]} className={cn('flex items-center gap-2 px-6 py-3 rounded-xl font-medium', practiced[currentTask] ? 'bg-rose-600 text-white hover:opacity-90' : 'bg-slate-100 text-slate-400 cursor-not-allowed')}>{currentTask < speakingTasks.length - 1 ? <><span>Следующее задание</span><ArrowLeft className="w-4 h-4 rotate-180" /></> : <><CheckCircle2 className="w-4 h-4" /> Завершить модуль</>}</button></div>

      {showSecrets && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" onMouseDown={() => setShowSecrets(false)}><div className="relative w-full max-w-md h-64 bg-white border border-slate-300" onMouseDown={(event) => event.stopPropagation()}><button type="button" onClick={() => setShowSecrets(false)} aria-label="Закрыть" className="absolute right-2 top-2 p-2 text-slate-500 hover:text-slate-800"><X className="w-4 h-4" /></button></div></div>}
    </div>
  );
}
...
function SpeakingRecorder({ task, points, onChecked }: { task: string; points: string[]; onChecked: (score: number) => void }) {
  ...
  const stopRecording = useCallback(() => { stopTimer(); setIsRecording(false); if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') mediaRecorderRef.current.stop(); }, [stopTimer]);
  stopRecordingRef.current = stopRecording;

  useEffect(() => () => { stopTimer(); streamRef.current?.getTracks().forEach((track) => track.stop()); if (audioUrl) URL.revokeObjectURL(audioUrl); }, [audioUrl, stopTimer]);
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      recorder.onstop = () => { const blob = new Blob(chunksRef.current, { type: recorder.mimeType || mimeType || 'audio/webm' }); setAudioBlob(blob); setAudioUrl(URL.createObjectURL(blob)); stream.getTracks().forEach((track) => track.stop()); streamRef.current = null; };
      recorder.start(250); setIsRecording(true);
      timerRef.current = setInterval(() => { setElapsed((value) => { const next = value + 1; if (next >= MAX_RECORDING_SECONDS) window.setTimeout(() => stopRecordingRef.current(), 0); return next; }); }, 1000);
  ...
  const resetRecording = () => { stopTimer(); if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') mediaRecorderRef.current.stop(); streamRef.current?.getTracks().forEach((track) => track.stop()); if (audioUrl) URL.revokeObjectURL(audioUrl); setAudioBlob(null); setAudioUrl(null); setResult(null); setError(''); setElapsed(0); setIsRecording(false); };
  ...
  const checkWithOtto = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/check-sprechen', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ task, points, audioBase64, mimeType: audioBlob.type, duration: elapsed }) });
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 mb-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4"><button onClick={isRecording ? stopRecording : startRecording} disabled={isChecking} aria-label={isRecording ? 'Остановить запись' : 'Начать запись'} className={cn('flex items-center justify-center w-16 h-16 rounded-full shrink-0', isRecording ? 'bg-rose-600 text-white animate-pulse' : 'bg-rose-50 text-rose-600 border-2 border-rose-200 hover:scale-105', isChecking && 'opacity-50 cursor-not-allowed')}>{isRecording ? <Square className="w-6 h-6" /> : <Mic className="w-7 h-7" />}</button><div className="flex-1"><div className="flex items-center gap-2"><Clock className="w-4 h-4 text-slate-400" /><span className="text-2xl font-bold text-slate-900 tabular-nums">{formatTime(elapsed)}</span><span className="text-xs text-slate-400">/ {formatTime(MAX_RECORDING_SECONDS)}</span></div><p className="text-sm text-slate-500 mt-0.5">{isRecording ? 'Идёт запись… нажмите ■, когда закончите.' : audioBlob ? 'Запись готова к проверке.' : 'Нажмите на микрофон и расскажите о себе.'}</p></div></div>
      {audioUrl && !isRecording && <div className="mt-4 p-3 rounded-xl bg-slate-50 border border-slate-200 flex flex-col sm:flex-row sm:items-center gap-3"><audio controls src={audioUrl} className="w-full min-w-0" /><button onClick={resetRecording} className="text-sm text-slate-500 hover:text-slate-700 whitespace-nowrap">Перезаписать</button></div>}
      ...
      {result && <div className="mt-5 space-y-4 animate-fade-in">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><div className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-2">Inhalt / Содержание</p><p className="text-sm text-slate-700">Названо: <strong>{result.covered.length}</strong></p>{result.missing.length > 0 ? <div className="mt-2 text-sm text-rose-700">Не хватает: {result.missing.join(', ')}</div> : <div className="mt-2 text-sm text-emerald-700">Все основные пункты выполнены.</div>}</div><div className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-2">Aussprache / Понятность</p><div className="flex items-center gap-2"><Volume2 className={cn('w-4 h-4', result.pronunciation.level === 'good' ? 'text-emerald-600' : 'text-amber-600')} /><p className="text-sm font-semibold text-slate-800">{result.pronunciation.title}</p></div><p className="text-xs text-slate-500 mt-1">{result.pronunciation.note}</p><p className="text-[11px] text-slate-400 mt-2">Это ориентир по уверенности распознавания, а не официальный фонетический балл.</p></div></div>
        ...
        {result.corrections.length > 0 && <div className="rounded-xl border border-slate-200 p-4"><p className="font-semibold text-slate-900 mb-3">Что улучшить</p><div className="space-y-3">{result.corrections.map((correction, index) => <div key={`${correction.original}-${index}`} className="rounded-xl bg-slate-50 p-3"><p className="text-sm text-red-700"><strong>Было:</strong> {correction.original}</p><p className="text-sm text-emerald-700 mt-1"><strong>Лучше:</strong> {correction.corrected}</p><p className="text-xs text-slate-500 mt-1">{correction.explanation}</p></div>)}</div></div>}
      ...
      {!audioBlob && !isRecording && !error && <div className="mt-4 flex items-start gap-2 text-xs text-slate-400"><Play className="w-3.5 h-3.5 mt-0.5 shrink-0" /><span>Запись остаётся в браузере до отправки. AI вызывается только после нажатия «Отправить Отто» — так мы не тратим деньги во время разговора.</span></div>}
    </div>
  );
}
