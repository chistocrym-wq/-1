...
type SourceTask = [title: string, situation: string, points: string[], pointsRu: string[]];
...
const allTasks: SchreibenTeil2Task[] = Array.from({ length: 80 }, (_, i) => {
  const [title, situation, points, pointsRu] = source[i % source.length];
  return { id: `t2-${String(i + 1).padStart(2, '0')}`, title: `${title} · Aufgabe ${i + 1}`, situation, points, pointsRu, minWords: 24, maxWords: 40 };
});

function basicCheck(text: string, task: SchreibenTeil2Task) {
  const lower = text.toLowerCase();
  const words = text.trim() ? text.trim().split(/\s+/).length : 0;
  const greeting = /(liebe|lieber|sehr geehrte)\b/i.test(text);
  const closing = /(viele grüße|liebe grüße|mit freundlichen grüßen|bis bald)/i.test(text);
  const points = task.points.map((point) => {
    const wordsToFind = point.toLowerCase().split(/[^a-zäöüß]+/).filter((w) => w.length > 3).slice(0, 2);
    return wordsToFind.some((w) => lower.includes(w));
  });
  const criteria = [greeting, ...points, closing, words >= task.minWords && words <= task.maxWords];
...
export function SchreibenTeil2Runner({ onBack }: { onBack: () => void }) {
  ...
  if (result) {
    return <div className="animate-fade-in">
      ...
      </div><p className="mt-4 text-xs leading-5 text-slate-500">Сейчас это предварительная проверка. Полная AI-проверка должна дополнительно оценивать содержание каждого пункта, грамматику, порядок слов и орфографию по памятке A1.</p>
  ...
  return <div className="animate-fade-in">
    ...
    <div className="mb-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-500">Schreiben · Teil 2</div><h3 className="mb-4 text-lg font-bold text-slate-900">{task.situation}</h3>{showRussian && <div className="mb-4 rounded-xl bg-slate-50 p-4 text-sm leading-6 text-slate-700"><strong>Русский перевод:</strong><br/>{task.situationRu || 'Перевод будет подключён автоматически через AI.'}</div>}<div className="rounded-xl bg-slate-50 p-4"><div className="mb-2 text-sm font-bold text-slate-800">Schreiben Sie zu jedem Punkt ein bis zwei Sätze.</div><div className="text-xs text-slate-500">circa 30 Wörter · Schreiben Sie auch eine Anrede und einen Gruß.</div><ul className="mt-3 space-y-2">{task.points.map((p,i)=><li key={i} className="flex gap-3 text-sm text-slate-700"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-amber-50 font-bold text-amber-700">{i+1}</span><span>{p}</span></li>)}</ul></div></div>
